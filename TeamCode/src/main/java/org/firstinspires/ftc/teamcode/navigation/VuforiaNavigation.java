...
public class VuforiaNavigation
{
    ...
    private LinearOpMode curOpMode;       // Access to the OpMode object
    private DriveSystem driveSys;        // Access to the Robot hardware
    ...
    public void addNavTelemetry() {
        if (targetFound)
        {
            ...
            curOpMode.telemetry.addData("Visible", targetName);
            curOpMode.telemetry.addData("Robot", "[X]:[Y] (B) [%5.0fmm]:[%5.0fmm] (%4.0f°)",
                    robotX, robotY, robotBearing);
            curOpMode.telemetry.addData("Target", "[R] (B):(RB) [%5.0fmm] (%4.0f°):(%4.0f°)",
                    targetRange, targetBearing, relativeBearing);
            curOpMode.telemetry.addData("- Turn    ", "%s %4.0f°",  relativeBearing < 0 ? ">>> CW " : "<<< CCW", Math.abs(relativeBearing));
            curOpMode.telemetry.addData("- Strafe  ", "%s %5.0fmm", robotY < 0 ? "LEFT" : "RIGHT", Math.abs(robotY));
            curOpMode.telemetry.addData("- Distance", "%5.0fmm", Math.abs(robotX));
        }
        else
        {
            curOpMode.telemetry.addData("Visible", "- - - -" );
        }
    }
...
//        driveSys.turnOrSpin(Y);
//        driveSys.setAxial(A);
//        driveSys.setLateral(L);
        DbgLog.msg("Yaw=%f, Lateral=%f, Axial=%f", Y, L, A);
     ...
     * @param driveSys     pointer to DriveSystem class
     */
    public void initVuforia(LinearOpMode opMode, DriveSystem driveSys) {
